...
of the follwoing exceptions when targeted at EL3:
...
-  Receive exceptions, but handle part of the exception in EL3, and delegate the
   ...
   ELs. In this scheme, the handling spans various secure ELs. See `execution
   delegation`__.

   .. __: `Execution delegation`_
...
for these non-interrupt exceptions to assume a prioirty, and to interwork with
...
Likewise, the a panic would result if it's attempted to deactivate a lower
priority level when a higher priority level is active.
...
priority of `delegated execution`__ in lower ELs. Delegated execution in lower
EL is associated with a priority level chosen with ``ehf_activate_priority()``
API (described `later`__). The chosen priority level also determines the
interrupts masked while executing in a lower EL, therefore controls preemption
...
-  ``ehf_activate_priority()`` activates a given priority, but only if the
   current active priority is less than the given one; otherwise panics. Also,
   in order to avoid preemption, the |EHF| programs the *Priority Mask Register*
   corresponding to the PE to the priority being activated. Dispatchers
   typically only need to call this when handling exceptions other than
   interrupts, and it needs to `delegate execution`__ to a lower EL at a desired
   priority level.
...
The introduction of |EHF| changes the behaviour:

-  For GICv3, the |EHF| always routes FIQs to EL3. This has a side effect of
   routing all Non-secure interrupts while in S-EL1 to EL3.

-  Non-secure interrupts while executing Secure world are always taken to EL3.
   So, dispatchers would miss the opportunity to populate the preempted return
   code.
...
.. [#irq] In case of GICv2, Non-secure interrupts while in S-EL1 were signalled
          as IRQs, and in case of GICv3, FIQs. Neither were routed to EL3, so
          were taken at the current exception level, i.e., S-EL1.

Execution delegation
--------------------

When whole or part of exception handling is performed at a EL lower than where
the exception targets, we call the handling delegated. For example, part of
handling of SErrors received at EL3 may be handled in lower Secure ELs.

EL3 executes with all exceptions masked, and therefore isn't preemptible.
Because |EHF| routes ``FIQs`` to EL3, execution in ELs below however can be
preempted.

As mentioned `earlier`__, dispatchers in EL3 might choose to delegate handling
of events to lower ELs. This delegated execution further might have requirements
of what interrupts are allowed to preempt it. The |EHF| facilitate this by
providing the APIs to `activate and deactivate`__ priority levels.

.. __: `delegation-use-cases`_
.. __: `ehf-apis`_

To this end, the dispatcher calls ``ehf_activate_priority()`` with the required
priority level before the dispatch, and calls ``ehf_deactivate_priority()`` with
the same priority level after the dispatch. The effect of calling these APIs
is subject to whether `priority transitions`__ are allowed.
